...
- Loads active, admin-approved job-source provider configs. Config drafts are
  generated by the external discovery service from public API docs, then
  smoke-tested before approval.
- Calls each approved provider endpoint with mapped query parameters.
