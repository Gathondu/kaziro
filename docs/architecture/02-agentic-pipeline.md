...
flowchart TD
  ...
  fetch["Job Fetch Service<br/>(approved provider configs)"]
  rawT[("raw_jobs")]
...
- Loads every admin-approved source config whose provider status is `ACTIVE`.
  Each scheduled or manually triggered job config fans out across all such
  providers. Config drafts are generated by the external discovery service
  from public API docs, then smoke-tested before approval.
- Calls each eligible provider endpoint with mapped query parameters.
- **Deduplication**: normalizes the application URL and hashes it into the raw
  job identity, falling back to the provider ID only when no application URL
  exists. The parser also reuses an existing per-user posting with the same
  normalized application URL. This protects against providers whose opaque IDs
  change between otherwise identical search results.
...
- **Manual document generation** (`POST /jobs/{id}/regenerate-documents`):
  requires a `GOOD_FIT` evaluation. A full generation refreshes or reuses the
  cached company research before creating both documents; partial CV or cover
  letter regeneration preserves the other saved document.
...
| Failure                            | Behaviour                                                                               |
| ---------------------------------- | --------------------------------------------------------------------------------------- |
| Provider API 5xx / timeout         | Celery `autoretry_for=(Exception,)` with exponential backoff, max 3 retries             |
