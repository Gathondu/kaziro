...
def create_jwt_token(user: AbstractBaseUser, token_type: TokenType) -> str:
    issued_at = _now()
    minutes = int(settings.AUTH_ACCESS_TOKEN_MINUTES)
    days = int(settings.AUTH_REFRESH_TOKEN_DAYS)
    ...
    return jwt.encode(dict(payload), settings.SECRET_KEY, algorithm="HS256")
