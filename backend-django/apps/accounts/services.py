...
async def signup(*, email: str, password: str, username: str) -> SignupResponse:
    ...
    log.info("auth.signup.created", user_id=str(user.id), confirmation_sent=confirmation_sent)
...
async def confirm_email(token: str) -> ConfirmationResponse:
    ...
    if user is None:
        raise BadRequestError(
            "Confirmation link is invalid or has expired.", code="invalid_confirmation_token"
        )
    if user.email_confirmation_expires_at and user.email_confirmation_expires_at < timezone.now():
        raise BadRequestError(
            "Confirmation link is invalid or has expired.", code="invalid_confirmation_token"
        )
...
async def resend_confirmation(email: str) -> bool:
    ...
    log.info("auth.confirmation_resent", user_id=str(user.id), confirmation_sent=sent)
    return sent
...
def issue_token_pair(user: User) -> TokenData:
    return TokenData(
        ...
        expires_in=int(settings.AUTH_ACCESS_TOKEN_MINUTES) * 60,
        user_id=user.id,
    )
...
async def _create_welcome_notification(user: User) -> None:
    ...
    await create_notification(
        ...
        payload={"type": "account_confirmed"},
    )
