...
const STEP_CONFIG: Record<ProfileOnboardingStep, StepConfig> = {
  ...
  summary: {
    ...
    description:
      "A short overview helps us tailor recommendations. Optional, so you can skip.",
    label: "Summary",
...
export function OnboardingProfileStep({
  ...
  const [value, setValue] = useState(() =>
    valueForField(draft.profile, config.field),
  );
  ...
  const primaryLabel =
    !config.required && value.trim().length === 0 ? "Skip" : "Next";
  ...
  function updateValue(next: string): void {
    ...
    setValue(
      config.kind === "number"
        ? sanitizeIntegerInput(next, EXPERIENCE_YEARS_MAX, 2)
        : next,
    );
  }
    ...
    <OnboardingStepFrame title={config.title} description={config.description}>
      <form className="space-y-4" onSubmit={submit}>
        ...
        <label className="block space-y-1.5">
          ...
          {fieldError ? (
            <span className="text-xs text-error">{fieldError}</span>
          ) : null}
        </label>
        <button
