...
export function OnboardingConfigStep() {
  ...
  const [fieldErrors, setFieldErrors] = useState<
    Partial<Record<(typeof CONFIG_ERROR_FIELDS)[number], string>>
  >({});
  ...
  const finishSetup = useMutation({
    mutationFn: async () => {
      if (!token) {
        throw new Error(
          "Your session has expired. Log in again to finish setup.",
        );
      }
      if (!cvFile) {
      ...
      const config = await createJobConfig(
        token,
        jobConfigPayloadFromFields(fields),
      );
  ...
  const scheduleOptions = schedules.data?.length
    ? schedules.data
    : FALLBACK_SCHEDULES;
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        <label className="block space-y-1.5">
          ...
          <input
            ...
            onChange={(event) =>
              updateField("keywordsText", event.target.value)
            }
        ...
        <label className="flex w-full items-start gap-3 rounded-xl border border-base-300 bg-base-200 px-3 py-2.5">
          <input
            ...
            onChange={(event) =>
              updateField("remote_only", event.target.checked)
            }
            type="checkbox"
          />
          ...
          <label className="block space-y-1.5">
            ...
            <input
              ...
              onChange={(event) =>
                updateField(
                  "salaryMinInput",
                  sanitizeIntegerInput(event.target.value, 999999999, 9),
                )
              }
              pattern="[0-9]*"
        ...
        <label className="block space-y-1.5">
          ...
          <select
            ...
            onChange={(event) =>
              updateField("fetch_schedule_cron", event.target.value)
            }
          ...
          {schedules.isError ? (
            <span className="text-xs text-warning">
              Using default schedules for now.
            </span>
          ) : null}
        </label>
        <button
