...
<div class="scroll-region flex min-h-0 flex-1 gap-3 overflow-x-auto overflow-y-hidden pb-1">
	{#each columns as col (col.status)}
		<div
			class="border-base-300 bg-base-200 flex h-full min-h-0 w-64 shrink-0 flex-col rounded-2xl border p-2"
		>
			<h3
				class="text-base-content/70 mb-2 shrink-0 px-1 text-xs font-semibold tracking-wide uppercase"
			>
				{col.status}
			</h3>
			<div
				class="bg-base-100/60 min-h-0 flex-1 space-y-2 overflow-y-auto rounded-xl p-2"
			...
			>
				{#each col.items as app (app.id)}
					<div
						...
						class="border-base-300 bg-base-100 rounded-xl border p-3 shadow-sm"
					>
						<a
						...
						>
							...
							<p class="text-base-content/60 text-xs">{app.job_posting?.company_name ?? ''}</p>
						</a>
					</div>
				{/each}
			</div>
